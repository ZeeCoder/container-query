...
test('should turn conditions to functions', () => {
    ...
    expect(getConditionFunction.mock.calls[0][0]).toEqual([ [ 'orientation', ':', 'landscape' ] ]);
    expect(getConditionFunction.mock.calls[1][0]).toEqual([ [ 'orientation', ':', 'portrait' ] ]);
    expect(getConditionFunction.mock.calls[2][0]).toEqual([ [ 'width', '>', 100 ] ]);
    expect(typeof processedConfig.queries[0].conditionFunction).toBe('function');
    expect(processedConfig.queries[0].conditionFunction()).toBe(1);
    expect(typeof processedConfig.queries[1].conditionFunction).toBe('function');
